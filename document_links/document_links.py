...
class DocumentLinks():
    ...
    def update_filetable(self,args):
        ...
        for file in local_file:
            if "M:\\" in file:
                self.db.open()
                file1 = file.replace("M:\\", "\\\\s-zera-stor01\data1\\")
                self.db.add_filePath(file1)
            
            if "K:\\" in file:
                self.db.open()
                file2 = file.replace("K:\\", "\\\\s-zera-stor01\data\\")
                self.db.add_filePath(file2)
            #else:
                ##self.db.open()
    ...
    def windows_check_links(self, args):
        ...
        for path_link in windows_tool_result:
            _path = path_link.rsplit(",", 1)[0]
            _link = path_link.rsplit(",", 1)[1]
            self.db.open()
            if self.db.contains_filePath(_link) == False:
                self.db.open()
                self.db.add_invalidLink(_path, _link)
        return True
